...
mod tests {
    ...
    fn run_test<D: Digest + 'static>() {
        let messages_as_hex: Vec<&str>;
        let expected_sha_output: Vec<&str>;

        let digest_is_sha256 = TypeId::of::<D>() == TypeId::of::<Sha256>();

        if digest_is_sha256 {
            messages_as_hex = get_sha256_test_messages();
            expected_sha_output = get_expected_sha256_outputs();
        } else {
            messages_as_hex = get_sha512_test_messages();
            expected_sha_output = get_expected_sha512_outputs();
        }

        for index in 0..5 {
            let byte_encoding = hex::decode(messages_as_hex[index]).unwrap();
            let sha_output: Vec<u8>;

            if digest_is_sha256 {
                sha_output = hash_input::<Sha256, 32>(vec![byte_encoding])
                    .unwrap()
                    .into();
            } else {
                sha_output = hash_input::<Sha512, 64>(vec![byte_encoding])
                    .unwrap()
                    .into();
            }
            assert_eq!(
                expected_sha_output[index].to_string(),
                hex::encode(sha_output)
            );
        }
    }
